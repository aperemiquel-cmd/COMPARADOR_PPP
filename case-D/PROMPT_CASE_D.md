# PROMPTS — Caso D: Codex (gestor) + Claude Code (revisor)

> Dos prompts secuenciales. Primero se ejecuta el prompt del gestor (Codex).  
> Solo cuando Codex declare el trabajo finalizado se ejecuta el prompt del revisor (Claude Code).

---

## PASO 1 — PROMPT GESTOR: Codex
...
Tu código será revisado por un segundo agente (Claude Code) que optimizará compatibilidad
y realizará refactoring. Para facilitar esa revisión:
  ...
  - Al finalizar, documenta en RESULTS_D.md:
...
## Registro ejecución — Gestor (Codex)
...
## PASO 2 — PROMPT REVISOR: Claude Code

> Ejecutar SOLO después de que Codex haya finalizado y hecho commit.  
...
Lee y comprende completamente los siguientes archivos ANTES de modificar nada:
  ...
  - /docs/STACK.md               → stack y entorno de despliegue
  - /case-D/RESULTS_D.md         → decisiones del agente anterior (LEER PRIMERO)
  - /data/FOT_COMPARADOR_PPPv11.csv → base de datos de productos

MISIÓN:
Recibes un proyecto ya construido por Codex en /case-D/.
Tu misión es revisarlo y mejorarlo. NO lo reescribas desde cero.

PROCESO OBLIGATORIO ANTES DE TOCAR NADA:
  1. Lee RESULTS_D.md para entender las decisiones del agente anterior
...
DOCUMENTACIÓN:
Añade una sección "## Revisión Claude Code" en RESULTS_D.md indicando:
...
## Registro ejecución — Revisor (Claude Code)
